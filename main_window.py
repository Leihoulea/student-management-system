from PyQt5.QtWidgets import QApplication, QMainWindow, QWidget, QVBoxLayout, QTableWidget, QGroupBox, QPushButton, QTabWidget, QLabel, QInputDialog, QMessageBox, QTableWidgetItem
...
class StudentManagementSystem(QMainWindow):
    ...
    def initUI(self):
        """
        初始化用户界面，设置窗口标题和大小，创建并设置中心小部件和布局。
        创建标签页用于不同管理模块的操作。
        """
    ...
    def init_course_tab(self):
        ...
        course_buttons_layout.addWidget(self.delete_course_button)
    ...
    def delete_course(self):
        """
        打开删除课程的对话框，允许用户输入课程ID以删除数据库中的相应课程记录。
        """
        self.delete_course_dialog = DeleteCourseDialog()
        self.delete_course_dialog.exec_()
...
if __name__ == '__main__':
    """
    程序的入口，创建 QApplication 实例和主窗口 StudentManagementSystem 并启动应用程序。
    """
