...
class Database:
    ...
    def add_major_change(self, student_id, old_major_id, new_major_id, change_date):
        ...
        # 检查专业是否已变更
        sql = "SELECT * FROM MajorChanges WHERE student_id = %s AND old_major_id = %s AND new_major_id = %s"
        self.cursor.execute(sql, (student_id, old_major_id, new_major_id))
        if self.cursor.fetchone():
            raise ValueError("Major change already exists for the given student")
