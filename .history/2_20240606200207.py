...
class Database:
    ...
    def update_student(self, student_id, name, gender, birthdate, major_id):
        # 检查学号是否存在
        sql = "SELECT * FROM Students WHERE student_id = %s"
        self.cursor.execute(sql, (student_id,))
        ...
        sql = "UPDATE Students SET name = %s, gender = %s, birthdate = %s, major_id = %s WHERE student_id = %s"
        self.cursor.execute(sql, (name, gender, birthdate, major_id, student_id))
        self.connection.commit()
...
class UpdateStudentDialog(QDialog):
    ...
    def submit(self):
        ...
        db = Database()
        try:
            db.update_student(student_id, name, gender, birthdate, major_id)
