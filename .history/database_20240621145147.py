...
class Database:
    ...
    def close_connection(self):
        if not self.is_closed:  # 检查连接是否已经关闭
            try:
                self.cursor.close()
                self.connection.close()
                self.is_closed = True  # 标记连接为已关闭
            except pymysql.MySQLError as e:
                raise RuntimeError(f"Failed to close the database connection: {str(e)}")
        else:
            print("Connection is already closed.")  # 打印连接已经关闭的信息
