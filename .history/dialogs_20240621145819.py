...
def get_connection(self):
    ...
    def update_student(self, student_id, name, gender, birthdate, photo_blob=None):
        """
        更新学生信息。如果提供了新的照片数据，更新照片。
        """
            ...
            with self.get_connection() as connection:
                with connection.cursor() as cursor:
                    ...
                    if photo_blob:
                        sql = """
                            UPDATE Students 
                            SET name = %s, gender = %s, birthdate = %s, photo = %s 
                            WHERE student_id = %s
                        """
                        cursor.execute(sql, (name, gender, birthdate, photo_blob, student_id))
                    else:
                        sql = """
                            UPDATE Students 
                            SET name = %s, gender = %s, birthdate = %s 
                            WHERE student_id = %s
                        """
                        cursor.execute(sql, (name, gender, birthdate, student_id))
                    
                    connection.commit()

        except pymysql.MySQLError as e:
            raise RuntimeError(f"Failed to update student information: {e}")

    # 可以对其他方法进行类似的改造

    def close_connection(self, connection):
        if connection and connection.open:  # 确认连接是打开的
            try:
                connection.close()
            except pymysql.MySQLError as e:
                print(f"Warning: Failed to close the database connection: {str(e)}")
修改 UpdateStudentDialog 类
在 dialogs.py 文件中，更新 UpdateStudentDialog 类以适应新的 Database 类。

python
复制代码
from PyQt5.QtWidgets import (
    QDialog, QFormLayout, QLineEdit, QPushButton, QFileDialog, QMessageBox, QLabel, QVBoxLayout
)
from PyQt5.QtGui import QPixmap
import re
from database import Database
...
class UpdateStudentDialog(QDialog):
    ...
    def check_student(self):
        ...
        try:
            ...
            QMessageBox.critical(self, 'Error', f"Failed to check student: {str(e)}")
        finally:
            if db:
                db.close_connection(db.connection)
    ...
    def show_update_form(self, student_info):
        ...
        self.layout.addRow('Gender:', self.gender_input)
        self.layout.addRow('Birthdate:', self.birthdate_input)
        self.layout.addRow('Photo:', self.photo_label)  # 显示照片
        self.layout.addWidget(self.photo_button)  # 添加选择照片按钮
    ...
    def choose_photo(self):
        """
        打开文件对话框，让用户选择新的照片，并预览选定的照片。
        """
        options = QFileDialog.Options()
        file_name, _ = QFileDialog.getOpenFileName(self, "Choose Photo", "", "Images (*.png *.jpg *.jpeg *.bmp);;All Files (*)", options=options)
        if file_name:
    ...
    def submit(self):
        ...
        # 处理照片更新
        photo_blob = None
        if self.selected_photo_path:  # 检查用户是否选择了新照片
            try:
                with open(self.selected_photo_path, 'rb') as photo_file:
                    photo_blob = photo_file.read()  # 读取新照片的字节数据
            except IOError as e:
                QMessageBox.critical(self, 'Error', f'Failed to read photo file: {e}')
                return

        db = Database()
        try:
            db.update_student(student_id, name, gender, birthdate, photo_blob)
    ...
    def submit(self):
        ...
        try:
            ...
            try:
                db.close_connection()
            except pymysql.MySQLError as e:  # 捕获 MySQL 关闭连接的错误
                print(f"Warning: Failed to close the database connection: {str(e)}")  # 打印警告信息，而不是显示错误对话框
    ...
    def display_student_info(self, student_info):
        ...
        for column_number, data in enumerate(row_data):
            if column_number == 5 and data:  # Display the photo
                photo_label = QLabel()
                pixmap = QPixmap()
                pixmap.loadFromData(data)  # 从 BLOB 数据加载图片
                pixmap = pixmap.scaled(50, 50, Qt.KeepAspectRatio)  # 调整图片大小
